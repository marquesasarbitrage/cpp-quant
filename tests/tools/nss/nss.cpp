...
void writeNelsonSiegelResult(const std::shared_ptr<NelsonSiegelFamily>& ns, std::map<double, double> initialData, std::string fileName) {

    // Open file for writing
    std::filesystem::create_directory("output-test3");
    std::ofstream file("output-test3/" + fileName +".csv");
    if (!file.is_open()) {
    ...
    CubicSpline cubicSpline(initialData);

    for (const auto& d:initialData){
        file << d.first << "," << d.second << "," << cubicSpline.evaluateSecondDerivative(d.first) << "," << ns->getRate(d.first) << "," << ns->getInstantaneousForwardRate(d.first) << "," << ns->getDerivativeInstantaneousForwardRate(d.first) << "," <<"\n";
    }

    file.close();
}

void testCanadianZeroYieldFit()
{
    std::cout << "Starting tests on canadian zero yield data (September 24th, 2025)." << std::endl;
    std::map<double, double> data = getCanadianZeroYieldData();
    NelsonSiegelCalibration nsCalib(data, true);
    writeNelsonSiegelResult(nsCalib.fitNelsonSiegel(), data, "canadaZeroCurveNelsonSiegel"); 
    writeNelsonSiegelResult(nsCalib.fitSvensson(), data, "canadaZeroCurveSvensson"); 
    std::cout << "Successful - find csv under build/output-test3 (canadaZeroCurveNelsonSiegel.csv and canadaZeroCurveSvensson.csv)." << std::endl;
}

int main()
{
    std::cout << "Starting test 3 - calibration procedure for Nelson Siegel and Svensson.." << std::endl;
    testCanadianZeroYieldFit();
    std::cout << "All Nelson-Siegel tests are over." << std::endl;
    return 0;
}
